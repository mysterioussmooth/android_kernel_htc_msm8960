...
static struct fb_info *msm_fb_get_primary_fb(void)
{
	int c = 0;
...
static int msm_fb_release(struct fb_info *info, int user)
{
	...
	if (!mfd->ref_cnt) {
		struct fb_info *fb0_info = msm_fb_get_primary_fb();
		struct msm_fb_data_type *mfd0 =
			(struct msm_fb_data_type *)(fb0_info? fb0_info->par:NULL);
		if (mfd->panel_info.type == DTV_PANEL &&
			(mfd0 && mfd0->suspend.panel_power_on == true)) {
			
			pr_warning("%s: skip DTV panel release during suspend\n", __func__);
		} else {
			if ((ret =
