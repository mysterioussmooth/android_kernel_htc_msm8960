...
static void m7_color_enhance(struct msm_fb_data_type *mfd, int on)
{
	if ((panel_type == PANEL_ID_M7_JDI_SAMSUNG) ||
		...
		printk(KERN_INFO "switch color enhance\n");
	} else if (panel_type == PANEL_ID_M7_SHARP_RENESAS_C1 ||
			panel_type == PANEL_ID_M7_JDI_RENESAS) {
		if (on) {
...
static void m7_sre_ctrl(struct msm_fb_data_type *mfd, unsigned long level)
{
	...
	if (prev_level != level) {
		...
		if (level >= 0 && level < 8000) {
			...
			current_stage = 11;
			printk(KERN_INFO "out of range of ADC, set it to 11 as default\n");
		}
		...
		printk(KERN_INFO "SRE level %lu prev_stage %lu current_stage %lu\n", level, tmp_stage, current_stage);
	}
}
...
void m7_set_cabc (struct msm_fb_data_type *mfd, int mode)
{
	...
	if (panel_type == PANEL_ID_M7_SHARP_RENESAS_C1 ||
		...
		printk(KERN_INFO "set_cabc mode = %d\n", mode);
       } else if (panel_type == PANEL_ID_M7_JDI_SAMSUNG_C2_1 ||
			panel_type == PANEL_ID_M7_JDI_SAMSUNG_C2_2) {
               if (mode == 2 || mode == 3) {
               ...
               mipi_dsi_cmdlist_put(&cmdreq);
	       printk(KERN_INFO "set_cabc strength = %d\n", mode);
	}
...
static int __init m7_panel_init(void)
{
	...
	if(panel_type == PANEL_ID_NONE)	{
		printk(KERN_INFO "%s panel ID = PANEL_ID_NONE\n", __func__);
		return 0;
	}
	...
	if (panel_type == PANEL_ID_DLXJ_SHARP_RENESAS) {
		...
		printk(KERN_INFO "%s panel ID = PANEL_ID_DLXJ_SHARP_RENESAS\n", __func__);
	} else if (panel_type == PANEL_ID_DLXJ_SONY_RENESAS) {
		mipi_video_sony_init();
		printk(KERN_INFO "%s panel ID = PANEL_ID_DLXJ_SONY_RENESAS\n", __func__);
	} else if (panel_type == PANEL_ID_M7_JDI_SAMSUNG) {
		mipi_command_samsung_init();
		printk(KERN_INFO "%s panel ID = PANEL_ID_M7_JDI_SAMSUNG\n", __func__);
	} else if (panel_type == PANEL_ID_M7_JDI_SAMSUNG_C2) {
		mipi_command_samsung_init();
		printk(KERN_INFO "%s panel ID = PANEL_ID_M7_JDI_SAMSUNG_C2\n", __func__);
	} else if (panel_type == PANEL_ID_M7_JDI_SAMSUNG_C2_1) {
		mipi_command_samsung_init();
		printk(KERN_INFO "%s panel ID = PANEL_ID_M7_JDI_SAMSUNG_C2_1\n", __func__);
	} else if (panel_type == PANEL_ID_M7_JDI_SAMSUNG_C2_2) {
		mipi_command_samsung_init();
		printk(KERN_INFO "%s panel ID = PANEL_ID_M7_JDI_SAMSUNG_C2_2\n", __func__);
	} else if (panel_type == PANEL_ID_M7_SHARP_RENESAS) {
		mipi_video_sharp_init();
		printk(KERN_INFO "%s panel ID = PANEL_ID_M7_SHARP_RENESAS\n", __func__);
	} else if (panel_type == PANEL_ID_M7_SHARP_RENESAS_C1) {
		mipi_cmd_sharp_init();
		printk(KERN_INFO "%s panel ID = PANEL_ID_M7_SHARP_RENESAS_CMD\n", __func__);
	} else if (panel_type == PANEL_ID_M7_JDI_RENESAS) {
		mipi_cmd_jdi_renesas_init();
		printk(KERN_INFO "%s panel ID = PANEL_ID_M7_JDI_RENESAS_CMD\n", __func__);
	} else {
		printk(KERN_ERR "%s: panel not supported!!\n", __func__);
		return -ENODEV;
	}

	pwmic_ver = i2c_smbus_read_byte_data(blk_pwm_client, 0x1f);
	printk(KERN_INFO "%s: PWM IC version A%d\n", __func__, pwmic_ver);
